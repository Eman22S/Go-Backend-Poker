...
Bootstrap.Popup = new Class({
	...
	_checkAnimate: function(){
		...
		if (!check) {
			...
			this._mask && this._mask.removeClass('fade').addClass('hide');
		} else if (check) {
			this.element.addClass('fade').removeClass('hide');
			this._mask && this._mask.addClass('fade').removeClass('hide');
		}
		return check;
	},
	...
	show: function(){
		...
		this._makeMask();
		this._mask && this._mask.inject(document.body);
		this.animating = true;
		if (this._checkAnimate()){
			this.element.offsetWidth; // force reflow
			this.element.addClass('in');
			this._mask && this._mask.addClass('in');
		} else {
			this.element.show();
	...
	hide: function(event, clicked){
		...
		if (this._checkAnimate()){
			...
			this._mask && this._mask.removeClass('in');
		} else {
			this.element.hide();
		...
		if (this.options.mask){
			if (!this._mask){
				...
				if (this.options.closeOnClickOut) {
							 this._mask.addEvent('click', this.bound.hide);
				}
				if (this._checkAnimate()){
