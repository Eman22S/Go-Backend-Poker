...
function ActionControlsAlternate({ on_new_table_state, disable_actions, ...props }) {
  ...
  useEffect(() => {
    const minimumRaise = getMinRaiseTo(
        tableState.game_meta.table_type,
        tableState.game.game_turn,
        tableState.game.bet,
        tableState.game.bigblind,
  ...
  const disable_call =
     // !canCall(tableState, store.player) || callBtnLoading;
  ...
  function onPlayerAction(action, bet_value, setBtnLoading) {
    ...
    grpc_client.sendPlayerAction(
      ...
      (tableStateResponse) => {
        setBtnLoading(false);
        const {action_result={}} = tableStateResponse;
        const {was_action_performed = true} = action_result;
  ...
  function getBtnCallText() {
    ...
    if (store.player.bet2do > 0) {
      call_str = `${call_str} ${toFloat( store.player.bet2do,
        store.numberFormat
      )}`;
    }
    return call_str;
          ...
          <TextFieldx
            ...
            inputProps={{
              style: {
                ...
                fontSize: "0.8715rem",
                padding: "0.5em",
                color: parseInt(minRaise) === parseInt(maxRaise) ? "black" : "primary"
              },
              readOnly: [minRaise === maxRaise]
            }} // html input element
            mt={1}
            mb={2}
          /> 
...
export default ActionControlsAlternate;
